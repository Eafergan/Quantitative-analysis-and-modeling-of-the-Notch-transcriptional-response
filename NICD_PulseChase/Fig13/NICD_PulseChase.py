...
path=('D:\\AOS\\1st_daptWashout\\1st_daptWashout_E\\input\\')
...
onlyfilesFullPath=['D:/AOS/1st_daptWashout/1st_daptWashout_E/output/channel1/'+string[:-8] + '.tiff' for string in onlyfiles ]
...
commandCh2 = 'ilastik.exe --headless --project="D:/work/ilastiks/NotchSignalOverNight_forbg/NotchSignalOverNight_forbg.ilp" --export_source="Simple Segmentation"  'f' {filesCH2}'
...
for x in range(len(onlyfiles)):
    ...
    for t in range(NumOfTReal):
        ...
        measureAllNotch2=tempmeasure2*tempNotchMask
        ...
        averageSigAllNotch2=np.sum(np.float64(Sig90AllofCh2))/np.sum(np.float64(tempNotchMask))

        resultsCh1[t,x]=averageSig1-averageBg1
        resultsCh2[t,x]=averageSig2-averageBg2
        resultsChAllofNotch2[t,x]=averageSigAllNotch2-averageBg2
        ...
        if t%6==0:
            print(t)
        del rawfile1, tempbgmaskNotch, tempbgmask,  rawfile2, tempmask, tempmeasure1,tempmeasure2,  measureMasked1, measureMasked2, averageSig1, averageBg1, averageSig2, averageBg2, bgMasked1, bgMasked2, Sig90Ch1, Sig90Ch2, Bg90Ch1, Bg90Ch2
...
y_pred = Single_exp_decay( vec,  C1Mat_sin,C2Mat_sin)
plt.plot(vec,y_pred,linestyle='dashed')
plt.plot(vec,normalized_matrix[ :, 2],linestyle='dashed')
plt.show()
...
#['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf'] color order
plt.plot(vec,y_pred,linestyle='dashed',color='#ff7f0e',label='Fit of two exponents')
plt.plot(vec,y_pred_single,linestyle='dashed', color='blue', label='Fit of single exponents')
plt.fill_between(vec, normalized_matrix[ :, 1]-ErrorMat[ :, 1], normalized_matrix[ :, 1]+ErrorMat[ :, 1],alpha=0.3,label='_nolegend_', color='#ff7f0e')
plt.legend(loc=3)
plt.ylim((0,1))
plt.show()
