...
for x in onlyfiles:
    ...
    for z in range(NumOfZ):
        ...
        tempimg3=rawfile.read(c=2,z=z,t=0,series=None,index=None,rescale=False,wants_max_intensity=False,channel_names=None,XYWH=None)
        bioformats.write_image(path[0:-6] + 'output\\Channel3_iRFP\\'+x[:-4]+'.tiff', tempimg3, PixelT, c=0, z=z, t=0, size_c=1, size_z=NumOfZ, size_t=1, channel_names=None)

        del tempimg1, tempimg2
    rawfile.close()
    del rawfile , xml_string, ome, iome, NumOfZ, PixelT
del x,path, onlyfiles
